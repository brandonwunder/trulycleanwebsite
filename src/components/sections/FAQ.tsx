  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border transition-all duration-200 ${
        isOpen
          ? 'bg-white shadow-md border-vibrant-teal/20'
      ...
      >
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${
            isOpen ? 'bg-vibrant-teal/10' : 'bg-gray-100'
          }`}>
            <span className={`font-heading font-bold text-sm ${isOpen ? 'text-vibrant-teal' : 'text-gray-400'}`}>
              {String(index + 1).padStart(2, '0')}
            </span>
          </div>
          <h3 className={`font-heading font-semibold text-base md:text-lg transition-colors ${
            isOpen ? 'text-vibrant-teal' : 'text-navy-dark'
          }`}>
            {item.question}
          </h3>
        </div>
        ...
        >
          <ChevronDown className={`w-5 h-5 transition-colors ${isOpen ? 'text-vibrant-teal' : 'text-gray-400'}`} />
        </motion.div>
      </button>
      ...
      <section id="faq" className="relative py-20 md:py-28 overflow-hidden bg-white">
        ...
        >
          ...
          <div className="text-center mb-16">
            ...
            >
              Got{' '}
              <span className="text-vibrant-teal">Questions?</span>
            </motion.h2>
            <motion.p
              variants={slideUp}
          ...
          >
            <div className="inline-flex items-center gap-3 bg-gradient-to-r from-vibrant-teal/5 to-vibrant-green/5 px-6 py-4 rounded-2xl border border-vibrant-teal/10">
              <HelpCircle className="w-5 h-5 text-vibrant-teal" />
              <p className="text-gray-600">
                Still have questions?{' '}
                <a href="tel:(602) 695-0607" className="text-vibrant-teal font-semibold hover:underline">
