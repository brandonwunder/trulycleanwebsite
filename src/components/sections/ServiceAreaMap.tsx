...
export default function ServiceAreaMap() {
  return (
    <section id="service-area" className="relative py-20 md:py-28 overflow-hidden bg-gradient-to-br from-white via-vibrant-teal/5 to-white">
      {/* Background blobs */}
      <motion.div
        className="absolute top-20 right-10 w-32 h-32 bg-vibrant-teal/5 rounded-full blur-3xl pointer-events-none"
        animate={{ y: [0, -20, 0] }}
      ...
      >
        ...
        <div className="text-center mb-16">
          ...
          >
            Proudly Serving{' '}
            <span className="text-vibrant-teal">North Phoenix</span>
          </motion.h2>
          <motion.p
            variants={slideUp}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <motion.div variants={slideRight} className="relative">
            <div className="relative bg-gradient-to-br from-vibrant-teal/5 to-vibrant-green/5 rounded-3xl p-8 border border-vibrant-teal/10">
            ...
            {serviceAreas.map((area, index) => (
              <motion.div
                ...
                className={`flex items-center gap-4 p-4 rounded-xl transition-all duration-200 hover:-translate-y-0.5 ${
                  area.primary
                    ? 'bg-gradient-to-r from-vibrant-teal/10 to-vibrant-green/5 border border-vibrant-teal/20 shadow-md'
              ...
              >
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${
                  area.primary ? 'bg-vibrant-teal/10' : 'bg-gray-100'
                }`}>
                  <MapPin className={`w-5 h-5 ${area.primary ? 'text-vibrant-teal' : 'text-gray-500'}`} />
                </div>
                <div>
                  <h3 className={`font-heading font-semibold ${area.primary ? 'text-vibrant-teal' : 'text-navy-dark'}`}>
                    {area.name}
                    {area.primary && (
                      <span className="ml-2 text-xs bg-vibrant-teal/10 text-vibrant-teal px-2 py-0.5 rounded-full">HQ</span>
                    )}
                  </h3>
